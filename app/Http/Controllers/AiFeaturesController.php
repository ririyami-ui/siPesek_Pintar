...
class AiFeaturesController extends Controller
{
        ...
        try {
            ...
            if ($user->role === 'teacher') {
                ...
                if (empty($data['schoolName'])) {
                    $data['schoolName'] = $adminProfile->school_name ?? 'Sekolah';
                }
            } else {
                if (empty($data['schoolName'])) {
                    $data['schoolName'] = $profile->school_name ?? 'Sekolah';
                }
            }
            ...
            if (empty($data['academicYear'])) {
                $data['academicYear'] = $profile->academic_year ?? '-';
            }
            if (empty($data['semester'])) {
                $data['semester'] = $profile->active_semester ?? 'Ganjil';
            }
    ...
    public function getQuizHistory(Request $request)
    {
        try {
            ...
            $quizzes = $query->latest()->take($limit)->get();
    ...
    public function getHandoutHistory(Request $request)
    {
        try {
            $limit = (int) $request->input('limit', 1000);
            $query = Handout::query();
            ...
            $handouts = $query->latest()->take($limit)->get();
    ...
    public function getWorksheetHistory(Request $request)
    {
        try {
            $limit = (int) $request->input('limit', 1000);
            ...
            $worksheets = $query->latest()->take($limit)->get();
