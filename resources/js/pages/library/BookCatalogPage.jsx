...
const BookCatalogPage = () => {
  ...
  const handleIsbnLookup = async () => {
    ...
    try {
      const response = await api.get(`/library/books/lookup/${formData.isbn}`);
          ...
          {books.map((book) => (
            <div key={book.id} className="group bg-white dark:bg-gray-800 rounded-[2rem] overflow-hidden border border-gray-100 dark:border-gray-700/50 shadow-sm hover:shadow-xl transition-all duration-500 flex flex-col">
              ...
              <div className="p-5 flex-1 flex flex-col">
                <div className="mb-4">
                  <h3 className="font-bold text-gray-800 dark:text-white line-clamp-1 group-hover:text-primary transition-colors" title={book.title}>
                    {book.title}
                  </h3>
                  <p className="text-sm text-gray-500 dark:text-gray-400 line-clamp-1">{book.author || 'Penulis Anonim'}</p>
                </div>

                <div className="grid grid-cols-2 gap-3 mb-6">
                  <div className="flex items-center gap-2 p-2 bg-gray-50 dark:bg-gray-900 rounded-xl">
                    <Layers className="text-primary" size={14} />
                    <div className="flex flex-col">
                      <span className="text-[10px] text-gray-400 uppercase font-bold">Stok</span>
                      <span className="text-xs font-bold text-gray-700 dark:text-gray-200">{book.available_stock}/{book.total_stock}</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 p-2 bg-gray-50 dark:bg-gray-900 rounded-xl">
                    <MapPin className="text-orange-500" size={14} />
                    <div className="flex flex-col">
                      <span className="text-[10px] text-gray-400 uppercase font-bold">Rak</span>
                      <span className="text-xs font-bold text-gray-700 dark:text-gray-200 truncate">{book.location || '-'}</span>
                    </div>
                  </div>
                </div>

                <div className="mt-auto flex items-center justify-between gap-2 border-t border-gray-50 dark:border-gray-700 pt-4">
                  <span className="text-[10px] font-mono text-gray-400">{book.isbn || 'Tanpa ISBN'}</span>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-2xl bg-white dark:bg-gray-800 rounded-[2.5rem] shadow-2xl overflow-hidden animate-zoom-in">
            <div className="p-8 pb-0 flex items-center justify-between">
              <div>
                <h3 className="text-2xl font-bold text-gray-800 dark:text-white">Tambah Koleksi</h3>
                <p className="text-sm text-gray-500">Scan ISBN untuk pengisian otomatis</p>
              </div>
              <button 
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-6">
              ...
              {formData.cover_url && (
                <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-2xl flex items-center gap-4">
                  <img src={formData.cover_url} alt="Cover Preview" className="h-16 w-12 object-cover rounded-lg shadow-sm" />
                  <div className="flex-1">
                    <p className="text-sm font-bold text-blue-800 dark:text-blue-300">Cover Otomatis Terdeteksi</p>
              ...
              <div className="flex gap-4 pt-4">
                ...
                <button 
                  type="submit"
                  className="flex-[2] py-4 bg-primary text-white font-bold rounded-2xl shadow-lg shadow-primary/20 hover:bg-primary/90 transition-all transform active:scale-95"
                >
                  Simpan Buku
                </button>
              </div>
