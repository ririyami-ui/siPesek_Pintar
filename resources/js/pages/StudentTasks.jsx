...
function TaskCard({ task }) {
  ...
  return (
    <div className={`p-3 rounded-xl border transition-all duration-300 hover:shadow-md shrink-0 w-[260px] relative flex flex-col ${
      isDone 
      ...
      {isDone && (
        <div className="absolute -top-2 left-1/2 -translate-x-1/2 z-10 w-5 h-5 bg-emerald-500 rounded-full border-2 border-white dark:border-slate-900 flex items-center justify-center shadow-md">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-1 mb-1">
          ...
          {!isGrade && task.date && (
            <span className="text-[9px] text-red-600 dark:text-red-400 font-bold bg-red-50 dark:bg-red-950/30 px-1.5 py-0.5 rounded-md border border-red-200 dark:border-red-800/30">
              {new Date(task.date).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })}
            </span>
          )}
        </div>

        <p className="font-bold text-slate-800 dark:text-white text-[10px] leading-snug line-clamp-2 mb-1.5 min-h-[2rem]">{task.topic}</p>

        <div className="flex items-center justify-between gap-2 mt-auto">
          <span className={`text-[8px] font-black uppercase tracking-wider ${isDone ? 'text-emerald-600' : 'text-slate-400'}`}>
            {task.status}
          </span>
          {isGrade && task.score > 0 && (
            <span className={`text-sm font-black ${isLow ? 'text-red-600 animate-pulse' : 'text-emerald-700 dark:text-emerald-400'}`}>
              {task.score}
            </span>
          )}
        </div>
...
export default function StudentTasks() {
  ...
  const visibleTasks = all.filter(t => {
    ...
    return s !== 'selesai' && s !== 'completed' && s !== 'complete';
  });
  const filtered = filter === 'all' ? visibleTasks : visibleTasks.filter(t => t.type === filter);
  
  const pendingGrades = visibleTasks.filter(t => t.type === 'grade' && (!t.score || t.score <= 0));
  const lowGrades     = visibleTasks.filter(t => t.type === 'grade' && t.score > 0 && t.score < 75);
  const pendingTasks  = visibleTasks.filter(t => t.type === 'task'  && t.status !== 'Selesai');
  
  const totalAttention = pendingGrades.length + pendingTasks.length + lowGrades.length;

  const totalGrade = visibleTasks.filter(t => t.type === 'grade').length;
  const totalTask  = visibleTasks.filter(t => t.type === 'task').length;
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 animate-fade-in-premium pb-20">
      ...
      {totalAttention > 0 && (
        <div className="bg-gradient-to-br from-red-600 to-orange-600 rounded-xl p-3 text-white shadow relative overflow-hidden">
          ...
          <div className="relative z-10 flex items-center gap-3">
            ...
            <div className="min-w-0">
              <div className="flex items-baseline gap-2">
                <p className="text-2xl font-black tracking-tighter">{totalAttention}</p>
                <p className="text-sm font-bold text-white/90">Hasil Belum Optimal</p>
              </div>
              <p className="text-[10px] text-white/70 mt-0.5 truncate">
                ...
                {lowGrades.length > 0 && `${lowGrades.length} Di Bawah Target`}
      ...
      {visibleTasks.length > 0 && (
        <div className="flex gap-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-xl w-fit mx-auto border border-slate-200 dark:border-white/5">
          {[
            { key: 'all',   label: `Semua` },
            ...
            <button
              ...
              className={`px-5 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all duration-300 ${filter === tab.key ? 'bg-emerald-600 text-white shadow' : 'text-slate-500 hover:text-slate-800 dark:hover:text-slate-300'}`}
            >
              {tab.label} {visibleTasks.length > 0 && <span className="ml-1 opacity-50">{tab.key === 'all' ? visibleTasks.length : (tab.key === 'grade' ? totalGrade : totalTask)}</span>}
            </button>
          ))}
        </div>
      )}
              ...
              <div key={sName} className="space-y-4">
                <div className="flex items-center justify-between px-2">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h2 className="text-sm font-black text-slate-800 dark:text-white uppercase tracking-tight">{sName}</h2>
                      <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">{items.length} Jejak Belajar</p>
                    </div>
                  </div>
                  ...
                  <div className="hidden sm:flex items-center gap-2 text-slate-300">
                    <ChevronLeft size={12} />
                    <span className="text-[9px] font-bold uppercase tracking-widest">Geser Timeline</span>
                ...
                <div className="relative">
                  ...
                  <div className="flex overflow-x-auto gap-3 pt-4 pb-4 px-1 no-scrollbar scroll-smooth">
                    {items.map((task, i) => (
                  ...
                  <div className="absolute top-0 right-0 bottom-0 w-12 bg-gradient-to-l from-white/90 dark:from-slate-900/90 to-transparent pointer-events-none z-10" />
                </div>
              </div>
            );
          })}
      ...
      {subjectNames.length === 0 && visibleTasks.length > 0 && (
